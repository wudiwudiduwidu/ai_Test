  ...
  <div id="addQuestionPage">
    ...
    >
      ...
      <a-form-item label="题目列表" :content-flex="false" :merge-props="false">
        <a-space size="medium">
          <a-button @click="addQuestion"> 底部添加题目</a-button>
          <!-- AI 生成抽屉 -->
          <AiGenerateQuestionDrawer
            :appId="appId"
            :onSuccess="onAiGenerateSuccess"
            :changeSubmit="handlerSubmitting"
            :submitting="submitting"
          />
        </a-space>
        <!-- 遍历每道题目 -->
        <a-collapse :bordered="false" :style="{ width: '600px' }">
          <a-collapse-item header="已生成题目">
            <div v-for="(question, index) in questionContent" :key="index">
              <a-space size="large">
                <h3>题目 {{ index + 1 }}</h3>
                <a-button size="small" @click="addQuestion">
                  添加题目
                </a-button>
                <a-button
                  size="small"
        ...
        <a-space size="large">
          <h3>题目 {{ questionContent.length + 1 }}</h3>
        </a-space>
        <a-form-item
          field="posts.post1"
          :label="`题目 ${questionContent.length + 1 + 1} 标题`"
        >
          <a-input v-model="newQuestion.title" placeholder="请输入标题" />
        </a-form-item>
        <!--  题目选项 -->
        <a-space size="large">
          <h4>题目 {{ questionContent.length + 1 }} 选项列表</h4>
          <a-button
            size="small"
            @click="addQuestionOption(newQuestion, newQuestion.options.length)"
          >
            底部添加选项
          </a-button>
        </a-space>
        <a-form-item
          v-for="(option, optionIndex) in newQuestion.options"
          :key="optionIndex"
          :label="`选项 ${optionIndex + 1}`"
          :content-flex="false"
          :merge-props="false"
        >
          ...
          <a-space size="large">
            <a-button
              size="mini"
              @click="addQuestionOption(newQuestion, optionIndex + 1)"
            >
              添加选项
            </a-button>
            <a-button
              size="mini"
              status="danger"
              @click="deleteQuestionOption(newQuestion, optionIndex as any)"
            >
              删除选项
            </a-button>
          </a-space>
...
const addQuestion = () => {
  questionContent.value = questionContent.value.concat(newQuestion.value);
  newQuestion.value = {
    title: "",
    options: [
      {
...
const handleSubmit = async () => {
  ...
  let res: any;
  // 把新添加的题目进入题目列表
  if (newQuestion.value.options.length > 0 && newQuestion.value.title) {
    questionContent.value = questionContent.value.concat(newQuestion.value);
  }
  // 如果是修改
...
const onAiGenerateSuccess = (result: API.QuestionContentDTO) => {
  questionContent.value = questionContent.value.concat(result);
};
const submitting = ref(false);
const handlerSubmitting = (submit: boolean) => {
