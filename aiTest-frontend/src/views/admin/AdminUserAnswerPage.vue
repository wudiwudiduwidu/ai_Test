<template>
  ...
  >
    <template #choices="{ record }">
      <a-button status="normal" @click="checkAnswer(record)"
        >查看用户选项</a-button
      >
    </template>
    ...
    <template #resultDesc="{ record }">
      <a-button status="normal" @click="checkResult(record)"
        >查看评分结果</a-button
      >
    </template>
...
const checkAnswer = (record: any) => {
  router.push({
    path: "/admin/userAnswer",
    query: {
      userAnswer: record.choices,
    },
  });
};
const checkResult = (record: any) => {
  router.push({
    path: "/answer/result/" + record.id,
    query: record.userId,
  });
};
</script>
